...
internal actual fun ApplyPlatformLanguage(tag: String?) {
    val original = systemDefault ?: ComposeLocale.current.platformLocale.also { systemDefault = it }
    ...
    if (ComposeLocale.current.platformLocale != target) Locale.setDefault(target)
}

public actual val PlatformSupportsLanguageOverride: Boolean = true
